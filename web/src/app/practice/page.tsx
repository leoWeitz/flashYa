...
const mockFlashcards = [
  ...
  },
  {
    ...
    concept: "Mitocondria",
    definition:
      "Orgánulo celular responsable de la respiración celular y la producción de ATP.",
  },
  {
    id: 3,
    concept: "ADN",
    definition:
      "Ácido desoxirribonucleico, molécula que contiene la información genética de los organismos.",
  },
];
...
export default function PracticePage() {
  ...
  const checkAnswer = () => {
    ...
    setFeedback({
      correct: isCorrect,
      message: isCorrect
        ? feedbackMessages[currentCardIndex].correct
        : feedbackMessages[currentCardIndex].incorrect,
    });
 ...
 ${
   listening
     ? "bg-green-500 w-full h-12 rounded-lg"
     : "bg-blue-500 hover:bg-blue-600 hover:scale-105"
            ...
            <Textarea
              ...
              className="resize-none rounded-lg overflow-hidden border-slate-200 bg-slate-50 text-slate-800 placeholder:text-slate-400 focus:border-blue-400 focus:ring-blue-400 min-h-[60px]"
            />

            <Button
              onClick={checkAnswer}
              disabled={!userAnswer.trim()}
              className="w-full rounded-lg bg-blue-500 hover:bg-blue-600 py-3 font-medium text-white mt-1"
            >
              Verificar respuesta
            </Button>
          </div>
